...
public class ItemSpeedBoat extends Item
{
        ...
        else
        {
            ...
            else
            {
                if (movingobjectposition.typeOfHit == EnumMovingObjectType.TILE)
                {
                    ...
                    EntitySpeedBoat entityspeedboat = new EntitySpeedBoat(par2World, (double)((float)i + 0.5F), (double)((float)j + 1.0F), (double)((float)k + 0.5F));
                    entityspeedboat.rotationYaw = (float)(((MathHelper.floor_double((double)(par3EntityPlayer.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3) - 1) * 90);

                    if (!par2World.getCollidingBoundingBoxes(entityspeedboat, entityspeedboat.boundingBox.expand(-0.1D, -0.1D, -0.1D)).isEmpty())
                    {
                        return par1ItemStack;
                    }
                    ...
                    {
                        par2World.spawnEntityInWorld(entityspeedboat);
                    }
	...
	{
		itemIcon = iconRegister.registerIcon("SpeedBoats:speedBoat");
	}
